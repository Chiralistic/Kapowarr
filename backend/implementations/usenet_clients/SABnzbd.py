...
class SABnzbd(BaseExternalClient):
    ...
    def add_download(self, download_link: str, target_folder: str, download_name: Union[str, None]) -> str:
        ...
        if is_direct_link:
            ...
            raise ClientNotWorking("Only direct NZB URLs are supported")

        if response.status_code != 200:
            raise ClientNotWorking(f"Failed to add download: {response.text}")

        data = response.json()
        if not data or 'status' not in data or data['status'] is False:
            error_msg = data.get('error', 'Unknown error')
            raise ClientNotWorking(f"Failed to add download: {error_msg}")
    ...
    def get_download(self, download_id: str) -> Union[dict, None]:
        ...
        if queue_response.status_code != 200:
            raise ClientNotWorking(f"Failed to get queue: {queue_response.text}")
        ...
        if history_response.status_code != 200:
            raise ClientNotWorking(f"Failed to get history: {history_response.text}")
